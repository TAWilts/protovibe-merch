...
const { t } = useI18n()
const currentStep = computed(() => nextSandboxTutorialStep(session.identity?.sandbox))
</script>

<template>
  <aside v-if="currentStep" class="sandbox-tutorial">
    ...
    <ol class="sandbox-tutorial-tasks">
      <li>
        <RouterLink :to="{ name: currentStep.route }" aria-current="step">
          <span class="sandbox-task-marker" aria-hidden="true">&#9675;</span>
          <span class="sandbox-task-copy">
            <strong>{{ t(`sandbox.tutorial.${currentStep.key}`) }}</strong>
            <span>{{ t(`sandbox.tutorial.${currentStep.key}Description`) }}</span>
          </span>
        </RouterLink>
      </li>
    </ol>
...
.sandbox-tutorial-tasks { margin: 0; padding: 0; list-style: none; }
